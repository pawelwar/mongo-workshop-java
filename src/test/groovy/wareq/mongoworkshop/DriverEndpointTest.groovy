...
class DriverEndpointTest extends Specification {
    ...
    def 'should get driver by the vehicle number'() {
        given:
        ensureDriversExist(STANISLAW_OLDEST_DRIVER, REGULAR_DRIVER, JULIA_OLD_DRIVER, JAN_OLD_DRIVER)

        when:
        Driver driver = getSingleDriver("/vehicles/${JULIA_OLD_DRIVER.vehicles.first().number}")

        then:
        driver?.id == JULIA_OLD_DRIVER.id
    }

    def 'the same vehicle number in many documents are not allowed'() {
        given:
        ensureDriversExist(REGULAR_DRIVER)
        String carNumberThatAlreadyExists = REGULAR_DRIVER.vehicles.first().number

        when:
        def driverWithDuplicateCarNumber = new Driver("some-id", "some-first-name", "some-last-name", 0, 20,
                [new Vehicle(carNumberThatAlreadyExists, "black", 10)]
        )
        ensureDriversExist(driverWithDuplicateCarNumber)
